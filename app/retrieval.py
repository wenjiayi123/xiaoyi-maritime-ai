...
class KnowledgeBase:
    def __init__(self, chunks: list[KnowledgeChunk] | None = None) -> None:
        ...
        self._search_token_frequencies = [
            Counter(tokens) for tokens in self._search_tokens
        ]
        self._keyword_sets = [set(chunk.keywords) for chunk in self.chunks]
        self._semantic_feature_sets = [
    ...
    def _bm25(self, query_tokens: list[str], document_tokens: list[str]) -> float:
        if not query_tokens or not document_tokens:
            return 0.0
        return self._bm25_from_frequencies(
            query_tokens,
            Counter(document_tokens),
            len(document_tokens),
        )

    def _bm25_from_frequencies(
        self,
        ...
        for term in set(query_tokens):
            ...
            denominator = frequency + k1 * (
                1
                - b
                + b * document_length / max(self._average_document_length, 1.0)
            )
            score += inverse * frequency * (k1 + 1) / denominator
        return score

    def _cached_bm25(self, query_tokens: list[str], index: int) -> float:
        return self._bm25_from_frequencies(
            query_tokens,
            self._search_token_frequencies[index],
            len(self._search_tokens[index]),
        )

    @staticmethod
    def _semantic_feature_set(text: str) -> set[str]:
        return _cjk_ngrams(text) | set(
    ...
    @classmethod
    def _semantic_similarity(cls, query: str, text: str) -> float:
        return cls._semantic_similarity_from_features(
            cls._semantic_feature_set(query),
            cls._semantic_feature_set(text),
        )
    ...
    def search_bm25_baseline(
        ...
        *,
        ...
        """Return an auditable BM25-only comparator for retrieval evaluation."""
        query_tokens = self._tokenize(query)
        evidence_terms = _evidence_query_terms(query)
        requested_jurisdictions = {
            str(item).strip().upper()
            for item in (jurisdictions or ())
            if str(item).strip()
        }
        hits: list[SearchHit] = []
        for index, chunk in enumerate(self.chunks):
            ...
            source_jurisdictions = {
                item.upper()
                for item in (chunk.provenance.jurisdictions or ("GLOBAL",))
            }
            if (
                requested_jurisdictions
                ...
                continue
            score = self._cached_bm25(query_tokens, index)
            if score <= 0:
                continue
            ...
            hits.append(
                SearchHit(
                    chunk=chunk,
                    score=round(score, 4),
                    snippet=_snippet(chunk.text, _extract_keywords(query)),
                    matched_terms=matched_terms,
                    coverage=coverage,
                    lexical_score=round(score, 4),
                    semantic_score=0.0,
                    rerank_score=round(score, 4),
                    retrieval_method="bm25_only_v1",
                )
            )
        hits.sort(
            key=lambda item: (item.rerank_score, item.coverage),
            reverse=True,
        )
        return hits[:top_k]
        ...
        for index, chunk in enumerate(self.chunks):
            ...
            for term in query_terms:
                ...
                if term_lower in title_lower:
                    ...
                    if weight >= 8.0:
                        score += 40.0
                if term in self._keyword_sets[index]:
                    score += weight * 0.5
            ...
            bm25_score = self._cached_bm25(query_tokens, index)
            semantic_score = self._semantic_similarity_from_features(
