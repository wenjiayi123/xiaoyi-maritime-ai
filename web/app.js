(() => {
  ...
  async function runHubEvaluation() {
    ...
    try {
      const result = await api("/api/evaluation/run", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({top_k:8}), timeoutMs:90000 });
      if ($("#hubHitMetric")) $("#hubHitMetric").textContent = `${Math.round(Number(result.hit_at_k || 0) * 100)}%`;
      ...
      toast("RAG 固定评测完成", `${result.benchmark_count} 条问题 · Hit@K ${Math.round(result.hit_at_k * 100)}% · 策略 ${Math.round(result.policy_safety_pass_rate * 100)}%`, result.passed ? "success" : "warning", 5200);
    } catch (error) {
