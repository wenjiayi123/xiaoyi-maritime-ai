...
# 启动就绪探针只检查服务身份与存活；深度联动状态仍由能力中枢调用
# /api/linkage/health。后者会聚合训练、仿真和审计状态，不适合作为 1.2s
# 的进程存活门禁，否则服务已在线时仍可能被误判为 STARTING。
export XIAOYI_ENERGY_HEALTH_URL="${XIAOYI_ENERGY_HEALTH_URL:-http://127.0.0.1:${ENERGY_BACKEND_PORT:-8808}/api/health}"
